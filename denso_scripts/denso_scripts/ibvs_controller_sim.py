...
# transformação óptico → basic_camera_link (mesma rotação definida no xacro da câmera)
R_OPT2PHYS = np.array([
    [ 0,  0,  1],
    [-1,  0,  0],
    [ 0, -1,  0],
...
class IBVSController(Node):
    def __init__(self):
        ...
        # ---- s* capturado na pose alvo com a câmera D405 simulada (1280x720) ----
        # Z no alvo: [0.123, 0.149, 0.146, 0.120] m
        self.s_star = np.array([
            0.045020202214891995, 0.3310456835065148,
            0.023916982426661373, -0.0631296419710098,
            0.41221622653010487, -0.09084509454364825,
            0.5247667320673348, 0.3064097256641695
        ])
        ...
        self.pub_error = self.create_publisher(Float64MultiArray, '/ibvs/error', 10)
        self.pub_twist = self.create_publisher(TwistStamped, CMD_VEL_TOPIC, 10)
    ...
    def features_callback(self, msg):
        ...
        if not (marker_detected and depth_valid):
            self.get_logger().warn(
                f'Dados não confiáveis (detected={marker_detected}, depth_valid={depth_valid}) '
                f'— pulando ciclo de controle', throttle_duration_sec=2.0)
            return

        if err_norm < self.threshold:
            self.stop_robot()
            self._converged = True
            self.get_logger().info(f'✅ CONVERGIU! ||e||={err_norm:.4f} < {self.threshold}')
            return
        ...
        v_lin_phys = np.clip(R_OPT2PHYS @ v[0:3], -MAX_L_VEL, MAX_L_VEL)
        v_ang_phys = np.clip(R_OPT2PHYS @ v[3:6], -MAX_A_VEL, MAX_A_VEL)
...
def main(args=None):
    ...
    try:
        ...
        try:
            ...
            pass   # contexto já encerrado; o Servo para sozinho por timeout
        node.destroy_node()
        if rclpy.ok():
            rclpy.shutdown()
